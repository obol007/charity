...
public interface DonationRepository extends JpaRepository<Donation, Long> {
    ...
    @Query(value = "select users.id, email, first_name, last_name, count(users.id) as donations from users join donation d on users.id = d.user_id group by users.id;",nativeQuery = true)
